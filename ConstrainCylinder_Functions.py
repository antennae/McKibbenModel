    ...
    for u in range(len(circle_ind_tab)):
        ...
        if len(cercle) > 2 : #on ne place les ressorts que si suffisamment de points sont alignés
            for ind_cercle in range(len(cercle)):
                ...
                NoeudCercle.addObject("SpringForceField", name="Springs" ,stiffness= stiffness,springsIndices1 =new_ind_tab[ind_0], springsIndices2 = new_ind_tab[ind_1] )#,length = d)# damping="4"
